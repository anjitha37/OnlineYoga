...
const loginUser = async (req, res) => {
  try {
    ...
    // ✅ Check for admin login (from .env)
    if (email === process.env.ADMIN_EMAIL) {
      const isMatch = await bcrypt.compare(password, process.env.ADMIN_PASSWORD_HASH);
      if (!isMatch) {
        return res.status(400).json({ msg: "Incorrect admin password" });
      }
      ...
      return res.status(200).json({
        ...
        token,
        user: { id: "admin_id", fullname: "Admin", email, role: "admin" }
      });
    }

    // ✅ Normal user/instructor login
    ...
    const token = jwt.sign(
      { id: loggedUser._id, role: loggedUser.role.toLowerCase() },
      process.env.JWT_SECRET,
      { expiresIn: "10d" }
    );
