...
export default function Loginpage() {
  ...
  const handleLogin = (e) => {
    ...
    axios
      .post(`${API_URL}/user/loginuser`, record)
      .then((res) => {
        alert(res.data.msg);
        if (res.status === 200) {
          localStorage.setItem("token", res.data.token);
          ...
          else if (role === "instructor") navigate("/instructorhome");
          else navigate("/userhome");
        }
      })
      .catch((err) => {
  ...
  const handleReset = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${API_URL}/user/reset-password/${token}`, { password: newPassword });
      setMessage(res.data.msg);
      setTimeout(() => {
        setMode("login");
        navigate("/login");
    ...
    <div className="login-wrapper">
      {mode === "login" && (
        <form className="form" onSubmit={handleLogin}>
          ...
          <div className="flex-row">
            ...
            <span className="span" style={{ cursor: "pointer", color: "blue" }} onClick={() => setMode("forgot")}>
              Forgot password?
            </span>
          </div>
      ...
      {mode === "forgot" && (
        <form className="form" onSubmit={handleForgot}>
          ...
          <p style={{ color: "green" }}>{message}</p>
          <p className="span" style={{ cursor: "pointer", color: "blue" }} onClick={() => setMode("login")}>
            ← Back to Login
          </p>
        </form>
      )}
